...
![Сеть: глобус группировки поворачивается, пока сутки идут с 06:00 до 08:24 — аппараты движутся по орбитам, маршрут клиента C65 перестраивается](docs/assets/screens/00-network.webp)

**[Открыть демо →](https://lpogosu.github.io/orbita-constellation/)** — интерфейс в браузере, без установки.

Решение кейса «Проектирование устойчивой спутниковой группировки» КосмоХакатона 2026.
Постановка, исходные данные и эталонный расчётный модуль принадлежат кейсодержателю и
в репозиторий не входят; ниже описано, как подставить эталон и повторить сверку.

## За пять минут

| Куда смотреть | Что там |
|---|---|
| [демо](https://lpogosu.github.io/orbita-constellation/) | тот же интерфейс в браузере: сеть, отказы, перебор, сравнение — на записанных ответах настоящего расчёта |
| [Интерфейс](#интерфейс) | экраны работающего сервиса: сеть на 3D-глобусе, разбор отказа, перебор, сравнение, телефон |
| [`docs/17_ARCHITECTURE_BRIEF.md`](docs/17_ARCHITECTURE_BRIEF.md) | архитектура решения одним документом |
| [`docs/19_ANALYSIS.md`](docs/19_ANALYSIS.md) | выводы и рекомендации по группировке с числами из расчётов сервиса |
| `make up` | весь стек локально одной командой, см. «Запуск» |

Команда, которая показывает, что расчёт верен:
...
Она сверяет 48 итоговых показателей по четырём сценариям кейса с таблицей
[`docs/10_FIXTURES.md`](docs/10_FIXTURES.md): 48 проверок, 0 расхождений.

Вторая проверка — сверка с официальным расчётным модулем кейсодержателя. Сам модуль
принадлежит организаторам, поэтому здесь его нет; положите выданный `geometry.py` в
`case/geometry/` (или укажите свой путь), и сверка выполнится:

```bash
python -m orbita_core crosscheck scenarios/01_full_constellation.json --ticks 0,120,43200,86280
```

На хакатоне она давала расхождение позиций 9,1·10⁻¹³ км и ноль расхождений в составе
линий связи. Без файла эта команда и зависящие от неё тесты пропускаются, а не падают.
...
Интерфейс спроектирован в Figma — от первого экрана до пустых, загрузочных и ошибочных
состояний — и перенесён в код один в один: координаты блоков на полотне совпадают с узлами
макета. Направление — тёмный северный космос с сиянием, стеклянные карточки, бирюзовые
рабочие связи, фиолетовые выбранные маршруты и коралловые отказы. Иллюстрации и
спутник-маскот отрабатывают пустые и успешные состояния, а в режиме расследования отказа
не появляются, чтобы не отвлекать. Главное требование, под которое собиралась сетка, —
высокая плотность данных без ощущения админки: карта, настройки, состояние сети и шкала
суток видны одновременно.

![Сеть: 3D-глобус группировки в момент 06:00, маршрут клиента C65, сводка конфигурации и шкала суток](docs/assets/screens/02-network-globe.webp)

| | |
|---|---|
| ![Отказы: сравнение сети до и после отказа двух аппаратов](docs/assets/screens/04-outages.webp) | ![Сеть в плоской схеме: все маршруты и орбитальные волны разом](docs/assets/screens/03-network-scheme.webp) |
| **Отказы.** Два аппарата выключены с утра: у каждого клиента видно, насколько упала доступность, где появился перерыв и почему | **Схема.** Равнопромежуточная проекция показывает все маршруты сразу; рядом режим «Ландшафт» с полярной картой и 3D |
| ![Исследования: тепловая карта перебора фазирования второй плоскости](docs/assets/screens/05-experiments.webp) | ![Сравнение трёх политик маршрутизации](docs/assets/screens/06-comparison.webp) |
| **Исследования.** Перебор конфигураций по двум осям сценария: тепловая карта, компромисс доступности и перерыва, топ-5 | **Сравнение.** Три политики маршрутизации на одном сценарии: дельты к базе, окна недоступности на общей оси, рекомендация |
| ![Итог расчёта: доступность клиентов, покрытие маршрутов, окна недоступности](docs/assets/screens/07-result.webp) | ![Сеть в светлой теме](docs/assets/screens/08-network-light.webp) |
| **Итог расчёта.** Доступность каждого клиента против цели, перерывы с причинами, выгрузка и Evidence Pack | **Светлая тема.** Тот же экран: темы отличаются только набором CSS-переменных |
| ![Проекты: загрузка сценария и примеры](docs/assets/screens/01-projects.webp) | |
| **Проекты.** Загрузка JSON с проверкой по схеме и четыре сценария кейса | |

<p align="center">
  <img src="docs/assets/screens/09-phone-network.webp" width="260" alt="Сеть на телефоне">
  <img src="docs/assets/screens/10-phone-outages.webp" width="260" alt="Отказы на телефоне">
  <img src="docs/assets/screens/11-phone-result.webp" width="260" alt="Итог расчёта на телефоне">
</p>

Снимки и анимация в шапке воспроизводимы: `python scripts/capture_screens.py` на поднятом
`make up` заводит проект, три расчёта, вариант с отказами и перебор через API и снимает
экраны — числа на них настоящие, а не из мокапа. Анимация собирается покадрово: на каждом
кадре сутки сдвигаются на один отсчёт сетки, глобус — на несколько градусов.

### Полотно и поток

Экраны нарисованы на полотне 1920×1080. На ноутбуке и мониторе полотно целиком
масштабируется под окно: пропорции блоков макетные, прокрутки нет, всё нужное для
расследования остаётся в одном кадре.

Ниже порога читаемости — телефон и планшет — интерфейс переходит в поток: те же
компоненты, стекло, иконки и типографика, но блоки выстраиваются колонкой, на планшете —
сеткой в две колонки; глобус и шкала суток понимают касания, широкие таблицы
прокручиваются внутри своих карточек. Порог задан не шириной окна, а коэффициентом
масштаба полотна (0.62): полотно ужимается по меньшей из сторон, и широкое, но низкое
окно ломается так же, как узкое. Решение принимает
[`web/src/app/viewport-mode.ts`](web/src/app/viewport-mode.ts), а экраны описывают
раскладку через `Slot`, `PageStack` и `useStacked()` — ветвление живёт в разметке каждого
блока, а не в общем CSS-сбросе координат, который ломал бы внутреннюю вёрстку карточек.

Читаемость на ноутбуке. Полотно ужимается целиком, и вместе с блоками мельчал бы текст: на
1366×768 подпись макета 12 px выходила бы на экран в 8.5 px. Поэтому координаты и
пропорции остаются макетными, а кегль мелкого текста компенсируется: подписи и капс не
мельче 10 px на экране, основной текст 13–15 px — не мельче 11–12 px, и рост ограничен
полутора макетными размерами. Граница никогда не больше самого размера, поэтому на
1920×1080 интерфейс совпадает с макетом пиксель в пиксель. Правило одно —
[`web/src/styles/readable-text.ts`](web/src/styles/readable-text.ts): его применяют
утилиты `text-*` Tailwind, подписи графиков и холст карты. Тесные места — колонки таблиц
на фиксированном шаге, подписи над полями — раздвигаются ровно на ту долю, на которую
вырос текст. На 1366×768 число надписей мельче 9 px на экране упало с 666 до нуля.

### Демо без бэкенда

[Демо](https://lpogosu.github.io/orbita-constellation/) — тот же интерфейс на GitHub Pages.
Ответы API в репозитории не хранятся: workflow
[`pages.yml`](.github/workflows/pages.yml) при каждом push в `main` поднимает весь стек,
считает проект из сценария кейса (три политики, отказ двух аппаратов, перебор 3×3),
проходит экраны Playwright'ом ([`scripts/record_demo.py`](scripts/record_demo.py)) и
публикует записанные ответы статикой. Поэтому демо всегда показывает поведение того кода,
из которого собрано.

Перехват живёт в одной точке — `web/src/api/client.ts`; экраны о демо не знают. Запросы на
чтение (включая POST сравнения, предпросмотра и критичности — ключ записи учитывает тело)
отдаются из записи; шкала суток воспроизводится по снимкам с шагом 6 минут, промежуточный
отсчёт получает ближайший записанный. Запуск расчётов, новые варианты и загрузка своих
сценариев отклоняются понятным сообщением — для них нужен полный стек, `make up`. В
обычной сборке кода демо нет вовсе.

### Токены и темы

Цвета, радиусы, тени, шрифты и шкала размеров — CSS-переменные
[`web/src/styles/tokens.css`](web/src/styles/tokens.css), подключённые к теме Tailwind в
[`web/tailwind.config.ts`](web/tailwind.config.ts). Компоненты пользуются ролями
(`bg-surface-raised`, `text-ink-secondary`, `rounded-2xl`), а не значениями, поэтому светлая
тема — это другой набор переменных на `<html data-theme>`, а не второй комплект стилей.
...
scripts/    воспроизводимые скриншоты интерфейса с локального стека
...
| `crosscheck` | сверка позиций и рёбер с эталонным модулем кейсодержателя; путь задаётся `--reference` |
...
Хакатонная версия сдавалась за двое суток. В этом репозитории она доведена до состояния,
которое не стыдно открыть с телефона или в инструментах разработчика.

- **Поток для телефона и планшета** — см. [«Полотно и поток»](#полотно-и-поток). На
  полотне при этом исправлены обрезанные подписи, наложения и невидимые статусы: часть
  классов ссылалась на цвета, которых в токенах нет.
- **Разбор устойчивости считался по BFS для любой политики.** В ядро уходило перечисление
  из HTTP-схемы, а ядро выбирает политику сравнением через `is` со своим перечислением.
  Теперь значение приводится к политике ядра, как в остальных сервисах, и тест проверяет
  именно тождество.
- **Экран «Отказы» бесконечно перезапрашивал расчёты** — десятки запросов в секунду, если
  адрес указывал на расчёт не последнего варианта. Автоподключение расчёта читало выбор,
  который само же и записывало, и два асинхронных подключения перекидывали его друг
  другу. Теперь расчёт подставляется один раз на вариант.
- **Тепловая карта перебора строилась по осям формы**, а не по осям посчитанного
  эксперимента, и ломалась после правки формы; ранжирование лучших точек считало нулевой
  перерыв худшим (`worst_max_gap_s or 10**9`).
- **Модальные окна выдёргивали фокус из поля ввода** на каждой перерисовке экрана.
- **Эталонный модуль организаторов убран из поставки.** Сверка с ним по-прежнему
  воспроизводима (`--reference`), зависящие тесты без файла пропускаются.
- **CI проверяет и фронтенд** (eslint, tsc, сборка); `mypy --strict` зелёный во всех трёх
  пакетах бэкенда.
