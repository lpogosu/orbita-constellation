...
|---|---|---|---|
| Действует | 2026-09-11 | таблица из `04_calculation_core.md` | Golden-значения получены независимым пересчётом на официальном `geometry.py` (`appendix/audit_2026-09-11.md`). Менять только при доказанной ошибке в `geometry.py` и с новым ADR |
...
16. При `min_elevation_deg = 90` наземных рёбер нет; при `isl_range_km → 0` ISL нет.
