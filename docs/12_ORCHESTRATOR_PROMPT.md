...
|---|---|---|---|
| Действует | 2026-09-12 | — | Сессия оркестратора запускается на Fable 5.1 (`/model`), рабочие агенты на Opus 5, ревьюер на Fable 5.1. Копируется в сессию-оркестратор. Рабочие агенты получают `11_AGENT_PROMPT.md` плюс одну карточку из §6. Ревьюер получает §8 и ничего из истории оркестратора |
...
`general-purpose`, модель `fable`, без worktree, на `main`. В его промпт входит только §8
